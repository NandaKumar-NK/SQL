create table dept (deptno int,dname nvarchar(20),constraint "PK_dept" primary key(deptno));
use sample;
create table emp(eno int,ename nvarchar(25)not null,ph int not null,deptno int , design nvarchar(20),
...
INSERT "Order Details" VALUES(3,2,14,12,0);
...
select avg(Unitprice) value from [Order Details];

select lower ('naNDa') name;

select upper(companyname) textdisplay from customers;

select count(*) countofprod from products where unitsinstock >20;

select sum(unitprice) cost from products;

select DATEADD(dd,23,getdate()) today;

select DATEDIFF(YY,birthDate,getdate()) today from Employees;

select len('hello world') aftertrim;
select substring('hello world',2,4) string;
select CHARINDEX('o','hello world') ind;
select replace ('h','j','hello world');